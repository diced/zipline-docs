...
Variables can be used in [Discord Webhook](/docs/guides/discord-webhook) messages and the [view route](/docs/guides/view-route) to customize the message with dynamic content.
...
Conditional modifiers follow this format: `{variableName.property::conditionalModifier["trueString"||"falseString"]}` (true and false string can also be empty strings (`""`)).
<br /><br />**Example**: `{file.maxViews::>0["{file.views}/{file.maxViews}"||"No max views"]}`.
<br />Using this example, when `file.maxViews` is greater than `0`, it'll show `{file.views}/{file.maxViews}` otherwise (when it's `0` or `null`) it'll show `No max views`.

| Modifier | Description                              | Types              | Variables                  |
| -------- | ---------------------------------------- | ------------------ | -------------------------- |
| `istrue` | If the boolean is true                   | `boolean`          | None                       |
| `exists` | Whether the element exists               | `string`           | None                       |
