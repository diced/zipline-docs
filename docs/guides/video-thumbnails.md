...
To enable it you have to set the [`FEATURES_THUMBNAILS`](/docs/config#features_thumbnails) environment variable to `true`.

## How?

Video thumbnails are generated on an interval. By default every 10 minutes (this can be configured), Zipline will search for video files (a mimetype of `video/*`) that do not have a thumbnail associated with them, and generate one for each.
