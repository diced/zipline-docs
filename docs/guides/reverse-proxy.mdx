...
```nginx filename="/etc/nginx/sites-available/zipline.conf" copy
server {
  listen 80;
...
```nginx filename="/etc/nginx/sites-available/zipline-ssl.conf" copy
server {
  listen 443 ssl;
  client_max_body_size 100M;
...
```nginx filename="/etc/nginx/sites-available/zipline-ssl.conf"
server {
  ...
