...
To log database queries and their parameters, enable debug logging and set `ZIPLINE_DB_LOG` to `true`:
...
Remove `ZIPLINE_DB_LOG` when finished. Query parameters can contain sensitive data, so review these logs before sharing them.
