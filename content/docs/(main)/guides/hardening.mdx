...
If you're running Zipline behind nginx, Caddy, or similar, enable **Trust Proxies** and configure **Trusted Proxies** with only your reverse proxies' IP addresses or CIDRs. Without both, Zipline uses the proxy's address instead of the client's, so clients behind it share rate limits. Restart Zipline after changing these settings.

For a proxy connecting over loopback on the same host:
...
For Docker, replace the loopback addresses with your actual proxy addresses or trusted proxy network. See [Reverse Proxy](/docs/guides/reverse-proxy#after-setting-up) for the full setup.
