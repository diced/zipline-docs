...
<Callout type='warning'>
  If startup reports "cannot safely migrate from prisma to drizzle", resolve the migration problem using the
  preceding release before retrying. Do not delete migration history or manually mark migrations as applied to
  bypass the check. The error identifies the missing, failed, or unexpected migration.
</Callout>
