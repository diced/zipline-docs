...
- a [postgresql](https://postgresql.org) server, or a local [PGlite directory](/docs/config/core#pglite)
...
After changing `src/lib/db/schema.ts`, run `db:migrate` to generate a migration in `drizzle/`. This command only creates the migration; Zipline applies committed migrations automatically on startup.
