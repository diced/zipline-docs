...
<Callout type='warning'>
  The export contains sensitive information, such as passwords, API keys, 2fa secrets, and more. Make sure to
  keep the file secure and delete it after the migration.
</Callout>
...
<Callout type='info'>
  ...
  run `ALTER DATABASE template1 REFRESH COLLATION VERSION;` in the postgres CLI to rebuild all of the objects
  in the template database using the new version, and you should be able to proceed with creating the
  database.
</Callout>
...
<Callout type='danger'>
  Zipline will fail to run if the database URL is the same as the one used in v3. This is because the database
  schema has changed significantly between v3 and v4.
</Callout>
