  ...
  return (
    <ScrollArea {...area} className={cn('min-h-0 flex-1', area?.className)}>
      <ScrollViewport
        {...viewport}
        className={cn(
          'scroll-styled *:flex! *:flex-col! *:gap-0.5! p-4 overscroll-contain mask-[linear-gradient(to_bottom,transparent,white_12px,white_calc(100%-12px),transparent)]',
          viewport?.className,
        )}
      >
        {children}
...
export function useAutoScroll(active: boolean, ref: React.RefObject<HTMLElement | null>) {
